...
public class PlateauView2Carcassonne extends JFrame implements MouseListener{
    
    public PlateauCarcassonne plateau;
    public ControleurCarcassonne controleur;
    

    public PlateauView2Carcassonne(PlateauCarcassonne plateau,ControleurCarcassonne controleur){ 
        super();
        ...
        for(int i=0; i<plateau.plateau.length; i++){
            for (int j=0;j<plateau.plateau[i].length;j++){
                if (!plateau.plateau[i][j].estRemplie()) {
                    ...
                    if(!plateau.plateau[i][j].pion){
                        TuileViewCarcassonne p=new TuileViewCarcassonne(plateau.plateau[i][j]);
                        this.getContentPane().add(p);
                    } else {
                        TuileViewPion p=new TuileViewPion(plateau.plateau[i][j],plateau.plateau[i][j].numJoueur);
                        this.getContentPane().add(p);
                    }
    ...
    public void update(){
        ...
        new ListeTuileViewCarcassonne(controleur.indiceJoueur,plateau);
    }
    public void tuile(JoueurCarcassonne j,int z,int x,int y){
        if(plateau.placerTuile(j,z,x,y)){
            plateau.plateau[x][y]=j.getMesTuiles()[z];
            plateau.plateau[x][y].pion=controleur.pion;
